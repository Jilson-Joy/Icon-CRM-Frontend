...
function ProductivityReport() {
  const { response, error, loading } = useAxios({
    method: 'GET',
    url: 'http://localhost:3022/productivityData' // Update URL with correct endpoint
  });
  const tablehead = [
    { id: 1, name: 'Employee' },
    { id: 2, name: 'Segment' },
  ...
  return (
    <NavbarSidebarLayout isFooter={false}>
      <div className="ProductivityReport_container">
        <h2 className="ProductivityReport_header">Productivity Report</h2>
        {loading ? (
          <p>Loading...</p>
        ) : error ? (
