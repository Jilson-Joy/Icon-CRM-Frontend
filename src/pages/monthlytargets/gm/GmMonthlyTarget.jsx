...
import './GmMonthlyTarget.css';
import { Button, Label } from 'flowbite-react';
import useGmMonthlyTarget from '../../../hooks/useGmMonthlyTarget';

function GmMonthlyTarget() {
  ...
  const { getGMMonthlyTarget, postGMMonthlyTarget } = useGmMonthlyTarget();

  useEffect(() => {
    const fetchSalesManagers = async () => {
      try {
        const data = await getGMMonthlyTarget();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await postGMMonthlyTarget(formData);
  ...
  return (
    <NavbarSidebarLayout isFooter={false}>
      <div className='gm_monthly_target_container'>
        <form onSubmit={handleSubmit} className='gm_monthly_target_form'>
          <h2 className="text-3xl text-center font-bold mb-6">Monthly Target - GM</h2>
          <div className="gm_monthly_target_input_box1 mb-2">
            <div className="gm_monthly_target_1">
              ...
              <select
                ...
                <option disabled selected>Sales Managers</option>
          ...
          <div className="gm_monthly_target_input_box1 mb-2">
            <div className="gm_monthly_target_1">
              <div className="mb-2 block">
                <Label htmlFor="profitTargetSales" value="Profit Target Sales" />
              </div>
              <input
                id="profitTargetSales"
                name="profitTargetSales"
                type="text"
                sizing="md"
                placeholder="Enter Profit Target Sales"
                className="w-full px-3 py-2 border rounded-lg focus:border-blue-500"
                value={formData.profitTargetSales}
                onChange={handleChange}
              />
            </div>
            <div className="gm_monthly_target_1">
              <div className="mb-2 block">
                <Label htmlFor="profitTargetService" value="Profit Target Service" />
              </div>
              <input
                id="profitTargetService"
                name="profitTargetService"
                type="text"
                sizing="md"
                placeholder="Enter Profit Target Service"
                className="w-full px-3 py-2 border rounded-lg focus:border-blue-500"
                value={formData.profitTargetService}
                onChange={handleChange}
              />
            </div>
            <div className="gm_monthly_target_1">
              <div className="mb-2 block">
                <Label htmlFor="profitTargetParts" value="Profit Target Parts" />
              </div>
              <input
                id="profitTargetParts"
                name="profitTargetParts"
                type="text"
                sizing="md"
                placeholder="Enter Profit Target Parts"
                className="w-full px-3 py-2 border rounded-lg focus:border-blue-500"
                value={formData.profitTargetParts}
                onChange={handleChange}
              />
            </div>
          </div>
...
export default GmMonthlyTarget;
